...
export function requireAuth(req, res, org) {
  ...
  // Embedded in the token at issuance (signup, login, verify-email) rather
  // than checked via a DB query here — this function runs on every single
  // authenticated request, so a stateless check consistent with how the
  // rest of this function already works. A token only ever carries
  // email_verified: false in the narrow window between signing up and
  // clicking the confirmation link; verifying mints a fresh token.
  if (payload.email_verified === false) {
    res.status(403).json({ error: 'email_not_verified', message: 'Please confirm your email address before continuing — check your inbox for the confirmation link.' });
    return null;
  }
