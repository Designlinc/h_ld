...
async function migrate() {
  ...
  // ── Invoices ──
  // invoice_number is assigned atomically from organizations.invoice_number_counter
  // at creation time (see lib/invoices.js) — gapless and sequential per org,
  // which matters for these to actually count as valid accounting records.
  // GST fields and ABN are snapshotted onto the invoice itself rather than
  // read live from settings at view time — if a practitioner's ABN or GST
  // registration changes later, past invoices must still show what was true
  ...
  `;
  await sql`ALTER TABLE organizations ADD COLUMN IF NOT EXISTS invoice_number_counter INTEGER NOT NULL DEFAULT 0`;
  ...
  await sql`ALTER TABLE bookings ADD COLUMN IF NOT EXISTS microsoft_event_id TEXT`;
  ...
  // 'pending' added for the paid-signup flow — a new org sits here from the
  // moment it's created until Stripe checkout actually completes (see
  // api/auth/signup.js and requireOrg in lib/tenant.js). The original CHECK
  // constraint predates that flow and didn't include it.
  await sql`ALTER TABLE organizations DROP CONSTRAINT IF EXISTS organizations_billing_status_check`;
  await sql`ALTER TABLE organizations ADD CONSTRAINT organizations_billing_status_check CHECK (billing_status IN ('active','trial','pending','suspended','cancelled'))`;

  await sql`ALTER TABLE invoices ADD COLUMN IF NOT EXISTS xero_invoice_id TEXT`;

  // ── External calendar events cache (2-way calendar sync) ──
  // Populated periodically by api/cron/sync-external-calendars.js from each
  // connected Google/Microsoft calendar. Holds events the practitioner
  // created directly in their native calendar app — NOT h_ld bookings,
  // which already live in the bookings table and are excluded from this
  // cache by the cron job. Purpose is twofold: (1) let the admin calendar
  // show these as greyed-out blocks with their real titles, for
  // visibility only — that title is NEVER exposed on the public booking
  // page; (2) let the public booking page treat these time ranges as
  // unavailable, same as it already treats existing bookings, so a
  // practitioner's other commitments actually block online booking
  // instead of just being visible after the fact. This cache is a
  // browsing convenience refreshed on a schedule, not a source of truth —
  // the authoritative check happens live, against the calendar provider
  // itself, at the moment a booking is actually confirmed (see
  // hasCalendarConflict in lib/calendarSync.js).
  await sql`
    CREATE TABLE IF NOT EXISTS external_calendar_events (
      ...
      PRIMARY KEY (practitioner_id, provider, event_id)
    )
  `;
  await sql`CREATE INDEX IF NOT EXISTS idx_external_events_practitioner_time ON external_calendar_events(practitioner_id, start_time, end_time)`;

  await sql`ALTER TABLE practitioners ADD COLUMN IF NOT EXISTS onboarding_completed BOOLEAN NOT NULL DEFAULT FALSE`;
  // One-time backfill, scoped by date rather than a blanket UPDATE — this
  // script re-runs on every deploy, so unconditionally setting everyone to
  // TRUE would also incorrectly mark a practitioner who signed up yesterday
  // and is only halfway through the wizard as "done" the next time any
  // unrelated migration runs. Scoping to accounts created before this
  // feature shipped means it only ever touches practitioners who were
  // already active before onboarding existed, permanently.
  await sql`UPDATE practitioners SET onboarding_completed = TRUE WHERE created_at < '2026-08-12' AND onboarding_completed = FALSE`;

  // Email verification — same safe date-cutoff backfill pattern as above.
  // token/expiry are cleared once used, and also naturally overwritten if
  // a fresh verification email is ever resent.
  await sql`ALTER TABLE practitioners ADD COLUMN IF NOT EXISTS email_verified BOOLEAN NOT NULL DEFAULT FALSE`;
  await sql`ALTER TABLE practitioners ADD COLUMN IF NOT EXISTS email_verify_token TEXT`;
  await sql`ALTER TABLE practitioners ADD COLUMN IF NOT EXISTS email_verify_token_expires TIMESTAMPTZ`;
  await sql`UPDATE practitioners SET email_verified = TRUE WHERE created_at < '2026-08-18' AND email_verified = FALSE`;
