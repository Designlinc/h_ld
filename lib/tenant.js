...
export async function requireOrg(req, res) {
  let org;
  try {
    org = await resolveOrgFromHost(req);
  } catch (err) {
    // resolveOrgFromHost's own DB call has no protection of its own, and
    // this function runs at the very start of nearly every API route in
    // the app, before that route's own error handling even begins. Without
    // this, a transient database hiccup here (the same kind already found
    // and fixed for invoice generation) would crash the whole request with
    // no JSON response at all — showing as a generic, unhelpful error
    // regardless of which endpoint was actually being called.
    console.error('requireOrg: failed to resolve organization', err);
    res.status(500).json({ error: 'Could not connect to the database — please try again' });
    return null;
  }
  if (!org) {
  ...
  if (org.billing_status === 'pending') {
    res.status(402).json({ error: 'payment_pending', message: 'Your account is almost ready — finish setting up billing to continue.' });
    return null;
  }
