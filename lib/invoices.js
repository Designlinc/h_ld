...
import { syncInvoiceToXero } from './xero.js';
...
export async function generateInvoiceForBooking(bookingId, orgId) {
  ...
  // Optional on top of everything above — if this practitioner hasn't
  // connected Xero, syncInvoiceToXero returns null immediately and this is
  // a no-op. If they have and it fails, that's worth knowing about but
  // must never undo the invoice that's already been created and emailed.
  try {
    await syncInvoiceToXero(invoice, booking.practitioner_id);
  } catch (err) {
    console.error('Xero sync failed for invoice', invoice.id, err);
  }
