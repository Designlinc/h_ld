...
export async function getValidXeroToken(practitionerId) {
  ...
  if (new Date(token.expires_at) > new Date(Date.now() + 60000)) {
    return { accessToken: token.access_token, tenantId: metadata.tenantId, defaultBankAccountId: metadata.defaultBankAccountId, defaultRevenueAccountCode: metadata.defaultRevenueAccountCode };
  }
  ...
  return { accessToken: data.access_token, tenantId: metadata.tenantId, defaultBankAccountId: metadata.defaultBankAccountId, defaultRevenueAccountCode: metadata.defaultRevenueAccountCode };
}
...
export async function syncInvoiceToXero(invoice, practitionerId, { force = false } = {}) {
  ...
  if (!xero.defaultRevenueAccountCode) {
    throw new Error('No revenue account configured for Xero — reconnect Xero in Settings to fix this');
  }
  // Refuses to blindly retry a sync that already succeeded once — reusing
  // the same InvoiceNumber against an invoice Xero already has (especially
  // one that's already had a payment applied) doesn't create a new
  // invoice, it tries to edit the existing one, which fails outright and
  // produces a confusing Xero validation error instead of a clear one.
  // Callers that genuinely want to try again (e.g. after voiding the
  // original in Xero) pass { force: true }.
  if (invoice.xero_invoice_id && !force) {
    throw new Error(`Already synced to Xero (invoice ${invoice.xero_invoice_id}). If that Xero invoice was voided or deleted, use "Force resync" to create a fresh one.`);
  }
  ...
  const invoiceRes = await fetch('https://api.xero.com/api.xro/2.0/Invoices', {
    ...
    body: JSON.stringify({
      Invoices: [{
        ...
        DueDate: issueDate,
        Status: 'AUTHORISED',
        // First-time sync: matches h_ld's own numbering exactly, so the
        // same invoice has the same number in both systems — easier to
        // reconcile than two unrelated sequences. Forced resync: omitted
        // entirely, since reusing the original number is exactly what
        // causes Xero to try editing the old (already paid) invoice
        // instead of creating a new one — let Xero assign its own number
        // for this one instead.
        ...(force ? {} : { InvoiceNumber: invoice.invoice_number }),
        Reference: force ? `${invoice.client_name} (resync of ${invoice.invoice_number})` : invoice.client_name,
        // h_ld's own total is already GST-inclusive (GST is backed out of
        // it, never added on top — see computeGst in lib/invoices.js).
        // Without this, Xero defaults to treating LineItem amounts as
        // tax-EXCLUSIVE and adds GST on top of a number that already
        // includes it, which is exactly what was causing the invoice to
        // show as short-paid even though the full amount had been applied.
        LineAmountTypes: 'Inclusive',
        LineItems: lineItems.map(li => ({
          Description: li.description,
          Quantity: li.quantity || 1,
          UnitAmount: li.unitPrice,
          AccountCode: xero.defaultRevenueAccountCode,
          // No TaxType specified on purpose — Xero's documented behaviour
          // is to fall back to whatever tax rate is already configured as
          // the default on this AccountCode, which is more correct than
          // this code guessing at a tax type string itself.
        })),
      }],
    }),
  });
  ...
  await sql`UPDATE invoices SET xero_invoice_id = ${xeroInvoice.InvoiceID} WHERE id = ${invoice.id}`;
