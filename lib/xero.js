  ...
  const invoiceRes = await fetch('https://api.xero.com/api.xro/2.0/Invoices', {
    ...
    body: JSON.stringify({
      Invoices: [{
        ...
        DueDate: issueDate,
        Status: 'AUTHORISED',
        // Matches h_ld's own numbering exactly, so the same invoice has the
        // same number in both systems — much easier to reconcile than two
        // unrelated numbering sequences. Reference carries the client's
        // name instead, since Xero shows both side by side and a name is
        // more useful there than a second copy of the number.
